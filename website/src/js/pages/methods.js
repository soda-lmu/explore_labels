...
const about = {
  ...
  "instruments.parquet": "One row per setup (84 LLM model × prompt recipes, human versions A–E, pooled humans) and label (OL, HS): prevalence with intervals, reliability, and confidence summaries.",
  ...
  "run_prevalence.parquet": "Share of positive labels in each LLM run (model × prompt recipe × run), for each label.",
  ...
  "llm_annotations.parquet": "Every LLM label: tweet, label type, model, prompt recipe, run, label, and confidence (where requested).",
